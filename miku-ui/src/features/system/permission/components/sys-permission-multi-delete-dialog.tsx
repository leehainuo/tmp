...
type SysPermissionMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function SysPermissionMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: SysPermissionMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const deletePromises = selectedRows.map((row) => {
        ...
        if (!permissionId) {
          return Promise.reject(new Error('权限ID不存在'))
        }
        return permissionApi.remove(permissionId)
      })

      await Promise.all(deletePromises)
      toast.success(`成功删除 ${selectedRows.length} 个权限`)
      ...
      title={
        <span className='text-destructive'>
          <AlertTriangle
            ...
            size={18}
          />{' '}
          删除 {selectedRows.length} 个权限
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            确定要删除选中的 {selectedRows.length} 个权限吗？ <br />
            此操作将永久删除这些权限，且无法恢复。
          </p>
